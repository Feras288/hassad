...
describe("تتبع الطلبات الحي", () => {
  ...
  it("يحمي تحديث الشحن بصلاحية المورد ويرسل إشعاراً للعميل", () => {
    ...
    expect(router).toContain(
      "أدخل رقم التتبع عند تحديث حالة الطلب إلى تم الشحن"
    );
  ...
  it("يربط إتمام الشراء وقائمة المورد وصفحة العميل بمصادر البيانات الحية", () => {
    ...
    const vendorOrders = projectFile(
      "client/src/pages/vendor/VendorOrders.tsx"
    );
    const customerOrders = projectFile(
      "client/src/pages/dashboard/DashboardOrders.tsx"
    );
    expect(checkout).toContain("trpc.orders.create.useMutation");
    expect(vendorOrders).toContain("trpc.orders.vendorList.useQuery");
    expect(vendorOrders).toContain(
      "trpc.orders.vendorUpdateTracking.useMutation"
    );
  ...
  it("يسجل طلب الإلغاء ويرسله للمورد ثم يعيد قرار المورد إلى العميل", () => {
    ...
    expect(customerOrders).toContain(
      "trpc.orders.requestCancellation.useMutation"
    );
    expect(vendorOrders).toContain(
      "trpc.orders.vendorReviewCancellation.useMutation"
    );
  });
  ...
  it("يقبل تقييماً واحداً موثقاً لتجربة التوصيل بعد التسليم فقط", () => {
    ...
    expect(database).toContain(
      "تم إرسال تقييم تجربة التوصيل لهذا الطلب مسبقاً"
    );
