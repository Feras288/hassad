...
describe("سوق المحاصيل B2B", () => {
  ...
  it("يتحقق من أطراف التفاوض ويمنع الطرف غير المصرح له من قراءة المحادثة أو الرد", () => {
    ...
    expect(router).toContain("quoteRequest.farmerId !== ctx.user.id");
    expect(router).toContain("quoteRequest.buyerId !== ctx.user.id");
  ...
  it("يوفر واجهات عروض المزارع وطلب اعتماد المشتري والتحكم الإداري بالخدمة", () => {
    ...
    const farmerPage = source(
      "client/src/pages/dashboard/DashboardProduce.tsx"
    );
  ...
  it("يعرض مراحل طلب التسعير بصورة مرئية متسقة للطرفين", () => {
    const quoteThread = source(
      "client/src/components/produce/ProduceQuoteThread.tsx"
    );
  ...
  it("ينشئ إشعارات خاصة بالطرف المقابل ويقيد القراءة بمالك التنبيه", () => {
    ...
    const alerts = source(
      "client/src/components/produce/ProduceQuoteAlerts.tsx"
    );

    expect(schema).toContain('"produce_quote_notifications"');
  ...
  it("يقبل مسار التخزين الداخلي الذي يعيده رفع صورة عرض المحصول", () => {
    ...
    expect(storage).toContain("url: `/storage/${key}`");
    expect(router).toContain('value.startsWith("/storage/")');
  ...
  it("يقتص صورة المحصول ويتيح تعيين الغلاف بنقرة واحدة", () => {
    const cropDialog = source(
      "client/src/components/produce/ProduceImageCropDialog.tsx"
    );
    const farmerPage = source(
      "client/src/pages/dashboard/DashboardProduce.tsx"
    );
  ...
  it("يعرض صور المحصول في عارض تفاعلي قابل للتكبير والتنقل والإغلاق", () => {
    const lightbox = source(
      "client/src/components/produce/ProduceImageLightbox.tsx"
    );
    const marketPage = source("client/src/pages/ProduceMarketplacePage.tsx");
